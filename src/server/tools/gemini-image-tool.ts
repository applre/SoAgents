...
function getGeneratedDir(): string {
  ...
  const dir = workspace
    ? join(workspace, 'soagents-generated', 'images')
    : join(homedir(), '.soagents', 'generated');
  if (!existsSync(dir)) {
    mkdirSync(dir, { recursive: true });
    // Ensure .gitignore in soagents-generated/ parent to prevent accidental commits
    if (workspace) {
      const parentDir = join(workspace, 'soagents-generated');
...
function getContextsDir(): string {
  const dir = join(homedir(), '.soagents', 'image-contexts');
