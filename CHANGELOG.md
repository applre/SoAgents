...
### 修复
- 切换工作区后最近对话列表不实时刷新（根本原因：Chat 组件每次 unmount→remount 导致 Sidecar 重启）
- 每次对话完成（`chat:message-complete`）后自动刷新 sessions，新对话实时出现在列表中
- `handleSessionsChange` 改为接受 `tabId` 参数，消除对 `activeTabId` 闭包的依赖，解决 sessions 写入错误 Tab 的问题
- 点击「新建对话」后 SoAgents Logo 上移的视觉抖动（侧边栏容器加 `overflow:hidden`，固定区块加 `shrink-0`）
...
### 新增
- **Phase 1 · 项目脚手架**：Tauri v2 + React 19 + Vite + TailwindCSS v4，暖纸质感设计系统
...
[0.2.0]: https://github.com/wangjida/soagents/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/wangjida/soagents/releases/tag/v0.1.0
