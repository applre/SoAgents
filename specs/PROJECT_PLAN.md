...
- Tauri v2 + React 19 + Vite + TailwindCSS 项目脚手架
- Rust 后端基础配置（logging、DevTools）
- 设计系统 CSS 变量（温暖纸张质感）
- TypeScript 严格模式配置
- Git 初始化
...
- macOS 自定义标题栏（交通灯留白 + 拖拽区域）
- 多 Tab 创建/切换/关闭
- Tab 拖拽排序 (@dnd-kit)
- Tab 重命名
- 快捷键：Cmd+T 新建、Cmd+W 关闭

#### Phase 3: Bun Sidecar 进程管理 ✅ 已完成

**交付物**：

- `sidecar.rs` — Rust 进程管理器 (Arc<Mutex>)
- 端口分配：31415 起递增
- 健康检查：60 × 100ms 轮询
- 进程生命周期：SIGTERM → 5s → SIGKILL
...
- `proxy.rs` — reqwest HTTP 代理（绕过 CORS）
- `tauriClient.ts` — Tauri IPC 封装
- `apiFetch.ts` — HTTP 代理封装
- `.no_proxy()` + `.tcp_nodelay(true)` 优化

#### Phase 5: SSE 代理 + 流式事件 ✅ 已完成

**交付物**：

- `sse.ts` — Bun SSE 服务（心跳 15s）
- `sse_proxy.rs` — Rust SSE 代理（事件路由）
- `SseConnection.ts` — 前端 SSE 接收
...
- 流式事件映射（SDK → SSE → React）
- `TabContext.tsx` + `TabProvider.tsx` — Tab 状态管理
- `Chat.tsx` — 对话页面
- `MessageList.tsx` + `Message.tsx` — 消息渲染

#### Phase 7: 完整对话体验 ✅ 已完成

**交付物**：

- 工具展示组件（Bash/Read/Write/Edit/Glob/Grep）
- `PermissionPrompt.tsx` — 权限弹窗
- `AskUserQuestionPrompt.tsx` — AI 提问交互
...
- `SessionStore.ts` — JSONL 持久化
- 9 种会话结束场景的状态重置
- `SessionHistoryDropdown.tsx` — 历史面板
- Token 统计（按模型分组）
- 会话切换与恢复
...
- `Settings.tsx` — 设置页面
- Anthropic 订阅模式 + API Key 模式
- 预设供应商（DeepSeek、Moonshot、智谱、MiniMax、火山引擎、OpenRouter）
- 自定义供应商 CRUD
- Provider 切换上下文保持（resume session）

#### Phase 10: MCP + Skills + 工作区 ✅ 已完成

**交付物**：

- MCP 服务器配置管理（STDIO/HTTP/SSE）
- Skills 系统（内置 + 自定义 + 同步）
- `WorkspaceFilesPanel.tsx` — 文件树
- 文件拖拽/粘贴上传
- `Editor.tsx` — Markdown/代码编辑器
- `Launcher.tsx` — 启动页
