...
SoAgents 的用户是需要 AI Agent 辅助工作的知识工作者：内容创作者、学生/研究者、产品经理、独立开发者、行业专家。典型使用场景是长时间桌面端使用，同时开多个 Tab 处理不同任务，频繁阅读 AI 生成的长文本。

### 品牌个性

**三词定位**：温暖 · 专业 · 高效

- **温暖**：干净纯白背景搭配暖棕强调色，营造舒适但不沉闷的氛围
- **专业**：清晰的信息层级，稳定可信赖的工具感
- **高效**：快速响应，不打断用户心流，信息密度适当

**情绪目标**：用户打开 SoAgents 应感到舒适和信任，像使用一款精心打磨的原生工具 — 干净、高效、不碍事。

### 美学方向

**视觉基调**：干净现代 (Clean Modern) — 纯白底色搭配暖棕强调色，在极简工具和精致创意应用之间

**色调演变**：早期采用暖奶油纸张质感（`#f6efe5`），现已转向纯白干净风格（`#FFFFFF`），更接近 Linear/Notion 的现代感。暖色调保留在强调色（`--accent: #c26d3a`）和语义色中，背景色走干净路线。

**参考方向**：
- **Apple 原生应用** — macOS 系统级原生质感、动效克制、信息层级清晰
- **Notion / Arc** — 温暖精致的强调色、创意工具的亲和力
- **Linear / Raycast** — 开发者工具的极简高效、键盘优先、信息密度

**反面参考（应避免）**：过度装饰/渐变泛滥的 Web3 风格、冰冷机械的纯灰色系（暖棕强调色是区分点）、信息过载的仪表盘风格

**主题模式**：当前仅浅色模式，深色模式作为未来规划项。设计系统已使用 CSS 变量，为主题切换做好基础准备。

### 设计原则

1. **阅读体验优先 (Reading First)** — AI 生成大量长文本，排版、行高、段落间距、内容宽度都围绕阅读舒适性设计
2. **层级清晰有重点 (Clear Hierarchy)** — 不同内容块有明确的视觉层级（L1-L5），用户能快速定位关键信息
3. **温暖但克制 (Warm but Restrained)** — 纯白背景保持干净，暖棕强调色点缀温度。颜色用于语义传达，不用于装饰。动效克制
4. **一致性即信任 (Consistency Builds Trust)** — 所有页面遵循相同设计语言。CSS 变量驱动，禁止硬编码颜色
5. **原生质感 (Native Feel)** — 追求接近 macOS 原生应用的质感。系统字体优先，尊重平台规范

### 无障碍

- 基础可访问性：合理对比度、键盘可访问
- 语义色搭配图标+文字，不仅靠颜色区分状态

---

## 设计理念

SoAgents 是一款 AI Agent 桌面客户端，采用**干净现代**的设计风格，纯白底色搭配暖棕强调色，营造舒适、专业的使用体验。
...
- **温暖亲和** - 纯白背景搭配暖棕强调色，干净而不冰冷，长时间使用不疲劳
...
| `--ink-tertiary` | `#999999` | 辅助文字、图标、placeholder |

#### Paper (背景色)
| Token | 值 | 用途 |
|-------|------|------|
| `--paper` | `#FFFFFF` | 主背景 |
| `--surface` | `#F7F7F7` | 输入框/次级区域背景 |
| `--hover` | `#EFEFEF` | 悬停态背景 |

#### Accent (强调色)
| Token | 值 | 用途 |
|-------|------|------|
| `--accent` | `#c26d3a` | 主强调色（暖棕） |
| `--accent-light` | `#e8a472` | 强调色浅色变体 |
| `--accent-warm` | `#c26d3a` | `--accent` 别名 |

#### Border (边框)
| Token | 值 | 用途 |
|-------|------|------|
| `--border` | `#E8E6E3` | 默认边框 |
| `--line-subtle` | `rgba(0, 0, 0, 0.06)` | 弱化边框、分割线 |
...
| Token | 值 | 背景色 | 用途 |
|-------|------|-------|------|
| `--success` | `#2e6f5e` | `#e6f0ec` | 成功、已启用、已完成 |
| `--error` | `#c25a3a` | `#fce8e3` | 错误、失败、危险操作 |
| `--warning` | `#c29a3a` | `#faf3e3` | 警告、需注意 |
| `--info` | `#4a7ab5` | `#e4ecf4` | 信息提示、加载中 |
...
按钮色未定义为独立 CSS 变量，通过组合已有 token 实现：

| 用途 | 实现方式 |
|------|---------|
| 主按钮背景 | `var(--accent)` (#c26d3a) |
| 主按钮文字 | `white` |
| 主按钮 hover | `opacity-90` |
| 次按钮背景 | `transparent` / `var(--surface)` |
| 次按钮边框 | `var(--border)` |
| 次按钮 hover | `var(--hover)` |
...
跨平台字体策略：使用 Inter 作为主字体，fallback 到系统字体。
...
**平台字体映射**：
| 用途 | 主字体 | macOS fallback | Windows fallback |
|------|--------|---------------|-----------------|
| 正文/标题 | Inter | -apple-system | Segoe UI |
...
圆角: var(--radius-lg)
内边距: var(--space-4)
Hover: border-color 加深至 var(--ink-tertiary)
```
...
描述: var(--text-sm), var(--ink-tertiary)
操作区:
  - 拒绝按钮: Ghost 样式
  - 允许按钮: Primary 样式
```
...
以下旧变量名已弃用，代码中如遇到应迁移至新名称：

| 旧变量 (design_guide v1.4) | 当前变量 (index.css) |
|---------------------------|---------------------|
| `--ink-muted` | `--ink-tertiary` |
| `--ink-subtle` | (已移除) |
| `--paper-elevated` | `--surface` |
| `--paper-inset` / `--paper-contrast` | `--hover` |
| `--paper-subtle` | (已移除) |
| `--accent-warm` | `--accent`（两者均存在，值相同） |
| `--accent-warm-hover` | `--accent-light` |
| `--accent-cool` | `--success`（语义不同，但值相同） |
| `--line` | `--border` |
| `--line-strong` | (已移除) |
...
```jsx
// 主按钮 (14px)
<button className="flex items-center gap-1.5 bg-[var(--accent)]
...
// 工具栏按钮 (13px) - Ghost 样式
<button className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5
  text-[13px] font-medium text-[var(--ink-tertiary)] transition-colors
  hover:bg-[var(--hover)] hover:text-[var(--ink)]">
  <Plus className="h-3.5 w-3.5" />
  新对话
</button>
...
<div className="bg-[var(--surface)] border border-[var(--border)]
  rounded-[var(--radius-lg)] p-4 hover:border-[var(--ink-tertiary)]
  transition-colors">
  卡片内容
</div>

// 输入框
<input className="bg-transparent border border-[var(--border)]
  rounded-[var(--radius-sm)] px-3 py-2.5 text-sm
  placeholder:text-[var(--ink-tertiary)] focus:border-[var(--ink)]
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 1.5.0 | 2026-03-24 | 色值同步 index.css 实际值；视觉基调从暖奶油转为纯白干净；token 命名对齐实现 |
| 1.4.0 | 2026-03-16 | 新增 Design Context：用户画像、品牌个性、美学方向、设计原则 |
